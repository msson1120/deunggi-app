...
st.title("📦 압축파일(.zip)로 등기부등본 Excel 일괄 분석")
st.markdown("""
업로드된 zip 파일 내의 .xlsx 문서를 자동 분석해 통합 결과를 생성합니다.
""")

uploaded_zip = st.file_uploader("📁 .zip 파일을 업로드하세요 (내부에 .xlsx만 포함)", type=["zip"])
...
if run_button and uploaded_zip:
    ...
    excel_files = [f for f in os.listdir(temp_dir) if f.lower().endswith(".xlsx")]
    ...
    for fname in excel_files:
        ...
        try:
            ...
            st.warning(f"{fname} 처리 중 오류 발생: {e}")
